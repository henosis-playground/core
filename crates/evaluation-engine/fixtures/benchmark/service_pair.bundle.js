  ...
  switch (wire.kind) {
    ...
    case "object": {
      ...
      for (const [name, child] of Object.entries(wire.fields)) {
        ...
        assertSchemaValue(makeSchema(child), object2[name], `${label}.${name}`);
      }
      return;
    }
  }
...
// ../../packages/platform-k8s/dist/index.js
var object = defineResource({
  kind: "k8s/object@1",
  outputs: {}
});
function emitObject(context, name, body) {
  return context.emit(object.create(name, body));
}
function emitServicePair(context, name, spec) {
  ...
  const env = spec.env === void 0 ? void 0 : Object.entries(spec.env).sort(([left], [right]) => left < right ? -1 : left > right ? 1 : 0).map(([entryName, entryValue]) => ({ name: entryName, value: String(entryValue) }));
...
var bundle = createBundle(k8s_services_default);
